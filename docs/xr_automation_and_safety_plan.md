...
## 17. Meta XR Simulator — the device path, on the PC

Every defect in section 16 was a one-line disagreement, and each one cost a hardware session to find: a build, an
`adb kill-server` dance, a headset on the head, and a logcat window filtered against a ring buffer VrApi is busy
overwriting. That loop is too slow and too tiring to iterate in, and it is the reason three of those four defects
shipped in the same build.

`tools/fake_quest.py` cannot shorten it. It speaks the wire protocol directly and never runs a line of the app, so
**every defect that lives on the device is invisible to it** — 16.1 is the clearest case, where fake_quest had been
sending the correct confirm gesture all along and the app was the odd one out.

Meta XR Simulator is an OpenXR runtime that stands in for a headset, so pressing Play in the editor runs
`TeleopBootstrap`, `OVRManager`, `OVRCameraRig`, the `UnityEngine.XR` input subsystem and `OVRPassthroughLayer` — the
real code paths, not a mock of them. Against the section 16 list it reaches 16.1 (the trigger is read for real), 16.2
(the console renders for real), 16.3 and 16.7 (a real head pose at a plausible height, against a real gate), and 16.6
(passthrough, against the simulator's synthetic rooms).

And the topology problem from 15.3 simply disappears. The host is on the same PC, so it is `127.0.0.1` with no Wi-Fi
bridge onto the robot network and no `adb reverse`. `Debug.Log` lands in the editor console.

### 17.1 Setup

```powershell
.\tools\simulator.ps1          # once after pulling; downloads the package
```

That assigns the Oculus XR loader for **Standalone** (`QuestBuild` only ever did Android, so without this
`TeleopBootstrap` sits through its ten-second wait for an active loader and then builds the rig with no tracking
behind it), pins the desktop graphics API to D3D11, and generates `Assets/Scenes/TeleopSimulator.unity` carrying
`TeleopBootstrap` — the same component the APK runs, deliberately **not** `TeleopPreviewBootstrap`, which mocks
exactly the parts the simulator exists to exercise.

D3D11 is not incidental: Meta's own note is that passthrough in the simulator needs an extra configuration parameter
set by hand when the app renders with Vulkan. Nothing needs Vulkan on the desktop, and looking at passthrough is half
the point. The Android player's graphics API is untouched.

Two packages were added to make this work: `com.meta.xr.simulator` 78.0.0, from the scoped registry the project
already uses and version-matched to the Core SDK, and `com.unity.test-framework`, because the simulator package
references NUnit and will not compile without it — which presents as `error CS0246: NUnit could not be found` coming
out of `PackageCache`, and is not obviously a missing-package problem the first time you see it.

`Assets/XR/` and `Assets/Scenes/` are both gitignored, so the loader assignment and the scene are regenerated by that
script rather than committed. Running it is a required step on a fresh clone, not a convenience.

### 17.2 The host

`teleop_hand_and_arm.py` cannot run on a development PC: it wants `pinocchio` for IK, which has no Windows wheel,
plus DDS and a robot. So the thing the simulator is most useful for — walking the align gate end to end while wearing
the console — was out of reach, which would have left 16.3 and 16.7 unverifiable anywhere but on hardware.

`tools/sim_host.py` is the smallest host that closes that. It runs the **real** `XrLinkServer`, the **real**
`openxr_to_robot` chain via `NativeXRSource`, and the **real** `AlignGate` with the real config, against forward
kinematics read straight out of `assets/g1/g1_body29_hand14.urdf` by walking joint origins with numpy. At zero joint
angles that reproduces `ArmFKMixin.forward_kinematics(q)` to 0.012 degrees, so the gate sees what it would see with a
real robot powered on and not yet moving.

What it deliberately is not: no IK, no `robot_arm.py`, no velocity limiter, no DDS, no gripper. **Nothing in it
commands a robot and nothing in it can.** It answers one question — would alignment have passed — and the answer is
real because the gate deciding it is the shipped one.

```
python tools/sim_host.py                 # align against the URDF initial pose
python tools/sim_host.py --pose ready    # a different pose, to watch it refuse
```

### 17.3 Verified this way already

The skip path, end to end, with no headset involved:

```
python tools/sim_host.py --port 8463
python tools/fake_quest.py --scenario align-skip --port 8463 --duration 12
```

```
  dir L  89.8 R  89.8  ok L0 R0  confirm 1 skip 1  k=1.28  hold  92%  hold... (position check skipped)
ALIGN ACCEPTED.
```

`confirm 1` is the trigger binding of 16.1 reaching the host. `skip 1` is X + A doing what the console claims. `k=1.28`
is the operator scale of 16.7 being derived from a real head height. `hold` climbing to acceptance is the gate that
could not be passed in build 13 passing. All of it in about ten seconds, on a desk.

`align-skip` is new to `fake_quest` and exists precisely because that combination — confirm gesture *and* both face
buttons, held continuously — is the one the old scenarios never produced, which is why nothing caught 16.1 before a
person in a headset did.

### 17.4 A bind failure was being reported as success

Found while building the above, and worth its own note because it wasted twenty minutes and would have wasted more
in the experience centre. `XrLinkServer.start()` waited on a readiness event that the server thread sets on **both**
...
`start()` never looked at anything but the event, so it returned `True` for a server that had never bound. The
symptom is a host that starts cleanly, prints that it is listening, and then waits forever for a device that cannot
possibly arrive — which reads as a headset or a network problem and is neither. The commonest cause is the port
already being held by a host that has not exited yet, which is now the normal way to trip it, since running two hosts
on one machine is what this whole section is about. `start()` now checks, logs the address it could not take, and
returns `False`.

### 17.5 What it still cannot tell you

Framerate. Real tracking noise. Real doff and don timing, which is the project's founding hazard and needs a
proximity sensor. Anything about the APK, the Android manifest, or the cleartext policy. Whether the robot moves.

It narrows what has to be checked on hardware to the things that genuinely need hardware. It does not replace the
bring-up order in 13.5, and section 15.3's warning stands: a fix verified only in simulation is not a fix verified.

---

## 18. Device telemetry — recorded, not reconstructed

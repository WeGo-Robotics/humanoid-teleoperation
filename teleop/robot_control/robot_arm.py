...
H1_2_Num_Motors = 35
H1_Num_Motors = 20
R1_Num_Motors = 35  # R1 uses unitree_hg LowCmd/LowState (same idl as G1/H1_2); active motors occupy idl indices 0-30
...
class R1_LowState:
    def __init__(self):
        self.motor_state = [MotorState() for _ in range(R1_Num_Motors)]
...
class R1_ArmController:
    ...
    R1 shares the unitree_hg LowCmd_/LowState_ idl and the rt/lowcmd / rt/lowstate
    topics with G1/H1_2, so it is driven through unitree_sdk2py exactly like the other
    arms. The C++ unitree_sdk2 r1 example is only required for R1's *high-level* loco
    client (walking); arm teleoperation is pure low-level and works from Python.

    R1 arms have 5 DOF each (shoulder pitch/roll/yaw, elbow, wrist roll) -> 10 DOF total.
    The logical R1 joint order is remapped to non-contiguous idl motor_cmd indices; the
    arm joints land on idl indices [15..19] (left) and [22..26] (right).
    """
    def __init__(self, motion_mode = False, simulation_mode = False):
        logger_mp.info("Initialize R1_ArmController...")
        self.q_target = np.zeros(10)
        self.tauff_target = np.zeros(10)
        self.motion_mode = motion_mode
        self.simulation_mode = simulation_mode
        ...
        # R1 low-level control uses rt/lowcmd. There is no rt/arm_sdk balance mode for R1,
        # so motion_mode is accepted for API symmetry but always publishes to rt/lowcmd.
        if self.motion_mode:
            logger_mp.warning("[R1_ArmController] motion_mode (arm_sdk) is not supported on R1; using rt/lowcmd.")
        self.lowcmd_publisher = ChannelPublisher(kTopicLowCommand_Debug, hg_LowCmd)
        self.lowcmd_publisher.Init()
        self.lowstate_subscriber = ChannelSubscriber(kTopicLowState, hg_LowState)
        self.lowstate_subscriber.Init()
        ...
        while not self.lowstate_buffer.GetData():
            time.sleep(0.1)
            logger_mp.warning("[R1_ArmController] Waiting to subscribe dds...")
        logger_mp.info("[R1_ArmController] Subscribe dds ok.")

        # initialize hg's lowcmd msg
        self.crc = CRC()
        self.msg = unitree_hg_msg_dds__LowCmd_()
        self.msg.mode_pr = 0  # R1 Mode::PR (series control for pitch/roll joints)
        self.msg.mode_machine = self.get_mode_machine()
        ...
        for idx, id in enumerate(R1_JointIndex):
            self.msg.motor_cmd[id].mode = 1
            self.msg.motor_cmd[id].kp = R1_Kp[id.value]
            self.msg.motor_cmd[id].kd = R1_Kd[id.value]
            self.msg.motor_cmd[id].q  = self.all_motor_q[idx]
        logger_mp.info("Lock OK!")
        ...
        logger_mp.info("Initialize R1_ArmController OK!")

    def _subscribe_motor_state(self):
        while True:
            msg = self.lowstate_subscriber.Read()
            if msg is not None:
                lowstate = R1_LowState()
                for id in range(R1_Num_Motors):
                    lowstate.motor_state[id].q  = msg.motor_state[id].q
                    lowstate.motor_state[id].dq = msg.motor_state[id].dq
                self.lowstate_buffer.SetData(lowstate)
            time.sleep(0.002)
    ...
    def _ctrl_motor_state(self):
        while True:
            ...
            for idx, id in enumerate(R1_JointArmIndex):
                self.msg.motor_cmd[id].q = cliped_arm_q_target[idx]
                self.msg.motor_cmd[id].dq = 0
                self.msg.motor_cmd[id].tau = arm_tauff_target[idx]
    ...
    def get_current_motor_q(self):
        '''Return current state q of all active R1 motors (order matches R1_JointIndex).'''
        return np.array([self.lowstate_buffer.GetData().motor_state[id].q for id in R1_JointIndex])

    def get_current_dual_arm_q(self):
        '''Return current state q of the left and right arm motors.'''
        return np.array([self.lowstate_buffer.GetData().motor_state[id].q for id in R1_JointArmIndex])

    def get_current_dual_arm_dq(self):
        '''Return current state dq of the left and right arm motors.'''
        return np.array([self.lowstate_buffer.GetData().motor_state[id].dq for id in R1_JointArmIndex])

    def ctrl_dual_arm_go_home(self):
        '''Move both arms to their home position by setting the target joint angles (q) and torques (tau) to zero.'''
        logger_mp.info("[R1_ArmController] ctrl_dual_arm_go_home start...")
        max_attempts = 100
        current_attempts = 0
        with self.ctrl_lock:
            self.q_target = np.zeros(10)
        ...
        while current_attempts < max_attempts:
            current_q = self.get_current_dual_arm_q()
            if np.all(np.abs(current_q) < tolerance):
                logger_mp.info("[R1_ArmController] both arms have reached the home position.")
                break
            current_attempts += 1
            time.sleep(0.05)
...
class R1_JointArmIndex(IntEnum):
    # idl motor_cmd indices for R1 arm joints (5 DOF per arm)
    # Left arm
    kLeftShoulderPitch = 15
    kLeftShoulderRoll = 16
    kLeftShoulderYaw = 17
...
class R1_JointIndex(IntEnum):
    # All active R1 motors, keyed by their idl motor_cmd index (joint_idx_in_idl in the
    # unitree_sdk2 r1 example). Non-arm joints are locked at their current position.
    # Left leg
    kLeftHipPitch = 0
    kLeftHipRoll = 1
    kLeftHipYaw = 2
...
# Stiffness / damping per idl motor index, taken from the unitree_sdk2 r1 example.
R1_Kp = {
    0: 200, 1: 200, 2: 200, 3: 200, 4: 200, 5: 200,        # left leg
    6: 200, 7: 200, 8: 200, 9: 200, 10: 200, 11: 200,      # right leg
    12: 300, 13: 300,                                       # waist
    15: 100, 16: 100, 17: 100, 18: 100, 19: 50,            # left arm (wrist roll = 50)
    22: 100, 23: 100, 24: 100, 25: 100, 26: 50,            # right arm (wrist roll = 50)
    29: 50, 30: 10,                                         # head
}
R1_Kd = {

...
class ImageClient:
    def __init__(self, host="192.168.123.164", request_port=60000):
        ...
        # ---- 핵심 수정: cameras 래핑 대응 ----
        if 'cameras' in self._cam_config:
            self._cam_config = self._cam_config['cameras']

        # ---- head_camera 필수 ----
        if 'head_camera' not in self._cam_config:
            raise KeyError(
                f"'head_camera' not found. Available keys: {list(self._cam_config.keys())}"
            )

        head = self._cam_config['head_camera']
        if head.get('enable_zmq', False):
            self._subscriber_manager.subscribe(self._host, head['zmq_port'])

        # ---- optional cameras ----
        for cam_name in ['left_wrist_camera', 'right_wrist_camera']:
            if cam_name in self._cam_config:
                cam = self._cam_config[cam_name]
                if cam.get('enable_zmq', False):
                    self._subscriber_manager.subscribe(self._host, cam['zmq_port'])

        if not head.get('enable_zmq', False) and not head.get('enable_webrtc', False):
            logger_mp.warning(
    ...
    def get_left_wrist_frame(self):
        if 'left_wrist_camera' not in self._cam_config:
            return None, 0.0
        return self._subscriber_manager.subscribe(
            self._host,
    ...
    def get_right_wrist_frame(self):
        if 'right_wrist_camera' not in self._cam_config:
            return None, 0.0
        return self._subscriber_manager.subscribe(
            self._host,
...
def main():
    ...
    while running:
        # ---- head camera ----
        if cam_config['head_camera'].get('enable_zmq', False):
            img, fps = client.get_head_frame()
            if img is not None:
                cv2.imshow("Head Camera", img)

        # ---- optional cameras ----
        if 'left_wrist_camera' in cam_config and cam_config['left_wrist_camera'].get('enable_zmq', False):
            img, fps = client.get_left_wrist_frame()
            if img is not None:
                cv2.imshow("Left Wrist Camera", img)

        if 'right_wrist_camera' in cam_config and cam_config['right_wrist_camera'].get('enable_zmq', False):
            img, fps = client.get_right_wrist_frame()
            if img is not None:
                cv2.imshow("Right Wrist Camera", img)
